...
const MAX_RELANCES_TRONCATURE = 1
const MAX_RELANCES_TRANSITOIRE = 3
const DELAIS_TRANSITOIRE = [700, 1800] // ms
const STATUTS_TRANSITOIRES = new Set([408, 429, 500, 502, 503, 504, 529])

// Marge minimale (ms) qu'il faut encore avoir devant soi pour tenter une
// generation supplementaire. En dessous, on rend ce qu'on a : depasser
// maxDuration = 60 s ferait un 504, que le client rejoue 3 fois — l'etudiant·e
// attendrait plusieurs minutes pour finir sur une erreur.
const MARGE_RELANCE_MS = 20000

const pause = (ms) => new Promise((r) => setTimeout(r, ms))

function tempsEcoule(deadlineAt) {
  return typeof deadlineAt === 'number' && Date.now() + MARGE_RELANCE_MS > deadlineAt
}
...
async function appelBrut({ system, prompt, model, maxTokens, deadlineAt }) {
  ...
  for (let tentative = 1; tentative <= MAX_RELANCES_TRANSITOIRE; tentative++) {
    try {
      ...
      if (tempsEcoule(deadlineAt)) throw err
...
export async function askClaude({ system, prompt, model = MODEL_DEFAULT, maxTokens = 1200, deadlineAt = null }) {
  let budget = maxTokens

  for (let essai = 0; essai <= MAX_RELANCES_TRONCATURE; essai++) {
    const { text, stopReason } = await appelBrut({ system, prompt, model, maxTokens: budget, deadlineAt })
    const tronque = stopReason === 'max_tokens'
    const vide = !text.trim()

    if (!tronque && !vide) return text

    if (essai === MAX_RELANCES_TRONCATURE || tempsEcoule(deadlineAt)) {
      ...
      console.error(
        `[anthropic] génération encore ${vide ? 'vide' : 'tronquée'} (budget ${budget}) — abandon des relances${tempsEcoule(deadlineAt) ? ' : plus assez de temps avant maxDuration' : ''}.`
      )
      return text
    }
